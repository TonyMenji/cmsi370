...
\abstract{In these tests, we went through eight participants and ran three tests on them with three different music media library programs to see which was best in each category of usability metrics and see which is best overall.}

\section{Introduction}
% JD: The first sentence is needs rewording---what do you mean by "three different music?"
...
% JD: Red flag: efficiency will not be explicit?  That seems odd because you seem
%     to be ideally positioned to measure that since you are already timing things
%     and, IIRC, some of your users are already proficient with the software tested.
...
% JD: You've been mixing tenses here.  Some are future tense, some are past tense.
%     Stay consistent---*did* you or *will* you conduct this study?.  Kind of a
%     big difference :-P
...
% JD: OK, interesting that you learned how to do multiple columns.  Not a big deal either
%     way, but at least you explored what you can do.
\begin{multicols}{2}
...
\end{itemize}
% JD: So my red flag on efficiency seems to be coming into play here.  You *did* measure
%     efficiency---the user took 2 minutes to perform this task.  The fact that you did
%     not recognize this as efficiency is...concerning.
...
\end{itemize}
% JD: OK, here it is fair to say that you are not measuring efficiency.  Same for any
%     user who has not used a system before.
...
% JD: Still, you timed this participant, right?  It's not clear to me
%     why you did not recognize this as an efficiency measure.
...
% JD: The past form of "find" in this context is "found," not "founded."

\begin{multicols}{2}
\subsection{1st Participant}

\subsubsection{\it iTunes}
% JD: Just use plain apostrophes (') and LaTeX will automatically make them curly.
...
\end{itemize}
\end{multicols}
% JD: As neat as the column-switching directive is, admittedly a table would have been
%     a much more compact way to report these results.
...
% JD: Yes, and the time to fulfill a task for someone who already knows the software is
%     known as.....?
...
\end{center}
\begin{center}
Figure 1: Average comparison for Learnability (*The numbers for iTunes in this chart are very skewed as only two participants have never used the program before the test. % JD: Unbalanced parenthesis here.  Does not send a good message about your proofreading.
\end{center}
% JD: Your bar graphs, while better than raw numbers, can still use some fine-tuning.
%     Note how you cluster the bars by product.  However, your text compares them by
%     task.  Thus, there is some disconnect between the visualization and the writing.
%     If we follow your text, it is easier to read the graph if you group the bars by
%     task, because that is what you are comparing.
...
Errors were made on the programs pretty consistently. These errors included things such as clicking on wrong items, going into wrong menus, going into wrong playlists  and even for those learning, it took them a lot of time to go through the program and learn what had to be done.
% JD: The above paragraph is a copy of the one right below your figure block.
%     What did you really intend to say here?
...
\begin{center}
\includegraphics[width=120mm]{Satisfaction.jpg} % JD: "Chart Title"???
\end{center}
\begin{center}
...
% JD: Interesting how, here, you have now clustered the bars in a way that is more
%     consistent with the text.  This is actually the better way for all of your
%     bar graphs.
...
% JD: Good that you recognize the distinction, but the conclusion you draw is completely
%     off.  What you should observe is that *depending on the user*, their timing data
%     is either learnability or efficiency.  Of *course* you can't take efficiency when
%     someone has not learned a program.  But you didn't see the *other* direction, which
%     is that when someone already does know how to use a program, the time that they take
%     to perform tasks *does* measure efficiency.

\subsection{Memorability}
Memorability was also a another metric that was difficult to get numbers for because of learnability issues with the programs and the users. Things that were noticed in these % JD: Whoa, sudden stop!

% JD: But even with the mid-sentence interruption, the text already shows the wrong
%     reason for the impracticality of measuring memorability: it's not because of
%     "learnability issues" (and while we're at it, what *is* a "learnability issue"
%     in the first place?)---it is because memorability requires some passage of time
%     between performing tasks, which is just not practical within the context of the
...
% JD: Which of your measured metrics is the most important?  You were asked to state
%     this.  Plus, this report still has barely any heuristic analysis (your conclusion
%     sort of starts heading that way but it's over quickly), and whatever analysis you
%     do have does not call out any of the course vocabulary at all (guidelines, principles,
%     mental models, etc.).
